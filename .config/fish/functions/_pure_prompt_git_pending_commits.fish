# Taken from github.com/rafaelrinaldi/pure
function _pure_prompt_git_pending_commits
    set --local git_unpushed_commits
    set --local git_unpulled_commits
    set --local pure_symbol_git_unpushed_commits "⇡"
    set --local pure_symbol_git_unpulled_commits "⇣"
    set --local git_unpushed_commits_color "#ffffff"
    set --local git_unpulled_commits_color "#ffffff"
    ...
    if test -n "$has_upstream"  # check there is an upstream repo configured
        ...
        if test $commit_to_push -gt 0  # upstream is behind local repo
            ...
            set git_unpushed_commits $pure_symbol_git_unpushed_commits
        end
        ...
        if test $commit_to_pull -gt 0  # upstream is ahead of local repo
            ...
            set git_unpulled_commits $pure_symbol_git_unpulled_commits
        end
    end

    printf "$git_unpushed_commits$git_unpulled_commits"
end
